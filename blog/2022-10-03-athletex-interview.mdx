---
title: "Developer story #1: Time series for sports prediction market"
author: Pei-Shan Wu
...
_This is a brand-new series: developer stories, for which we interviewed
different developers in our community for their actual experience building with
QuestDB. For the first post of this series, we interviewed Kevin Kamto,
Co-founder at [AthleteX Markets](https://www.athletex.io/)._
...
AthleteX Markets. That means that on a day to day basis, I operate as head of
sales, and a point of coordination for the team.
...
time? As someone who loves sports like soccer and basketball, I imagine that if
I could put my money towards an athlete, I could keep it safe.

:::note

This blog post is not an investment advise. 

:::

The idea of AthleteX is that you can invest in the performance of your favourite
...
runs are generated every minute or every five minute. We must store all that
...
fast, we need a database to manage large time series data sets and provide fast
queries time.
...
calculate the book value based on the WAR formula, and store it in QuestDB.
QuestDB is the base layer that allows us to handle this kind of throughput. At
the moment, we're storing nearly 2 millions records in QuestDB.
...
Notes from the QuestDB team: Since
...
doesn't appear to be as human friendly as Metabase, especially for the
non-technical end users that we have.

### Anything more you'd like to tell the readers?

AtheleteX 1.0.0 has just been released on Sept 10. If you're interested learning
more about the project, check out
